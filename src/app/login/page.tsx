...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Handle no-access users
            if (data.data.noAccess) {
                router.push('/no-access');
                return;
            }

            // Handle context picker for multi-role users
            if (data.data.requireContext) {
                // Store contexts and password for the select page to re-auth
    ...
    const handleReset = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const data = await res.json();
            setResetMessage(data.data?.message || 'Se o email existir, um link de recuperação foi enviado.');
        } catch {
                ...
                <div className="login-card">
                    ...
                    {!showReset ? (
                        <form onSubmit={handleLogin}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', textAlign: 'center', marginTop: 'var(--space-4)' }}>
                        ...
                        <div>
                            <p style={{ fontSize: 'var(--font-size-sm)', color: 'rgba(255,255,255,0.6)', marginBottom: 'var(--space-4)' }}>
